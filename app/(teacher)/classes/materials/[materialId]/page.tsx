...
type ManagerMaterialViewerPageProps = {
  params: Promise<{
    materialId: string;
  }>;
...
export default async function ManagerMaterialViewerPage({ params, searchParams }: ManagerMaterialViewerPageProps) {
  ...
  if (!profileResult.ok) {
    return (
      <main className="mx-auto min-h-screen max-w-[90rem] px-6 py-12">
        ...
        <BackTextLink className="mt-4" href="/login">
          Quay lại đăng nhập
        </BackTextLink>
      </main>
    );
  ...
  const classContextResult = query.classId
  ...
  const materialPanel = !materialResult.ok ? (
    <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700" data-testid="manager-material-error">
    ...
    <section className="space-y-4 rounded-lg border border-slate-200 bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-900" data-testid="manager-material-title">
        ...
        {materialResult.data.allowDownload && materialResult.data.downloadUrl ? (
          <a
            ...
            data-testid="manager-material-download-link"
            href={materialResult.data.downloadUrl}
          >
            Tải xuống
          </a>
        ) : (
          <span className="text-sm text-slate-500" data-testid="manager-material-download-disabled">
            Tải xuống đã bị tắt.
          </span>
        )}
      </div>
      ...
      <iframe
        ...
        data-testid="manager-material-viewer"
  ...
  return (
    <main className="mx-auto min-h-screen max-w-[90rem] px-6 py-12">
      <BackTextLink className="mb-4" href={managerBackHref}>
        {managerBackLabel}
      </BackTextLink>
      <div className="mb-6">
