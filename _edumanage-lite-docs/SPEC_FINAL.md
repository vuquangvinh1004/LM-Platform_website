...
| `teacher` | Gửi yêu cầu mở lớp, quản lý lớp mình phụ trách, tạo tài liệu, tạo bài kiểm tra và chọn câu hỏi khả dụng từ ngân hàng đề học phần, xem kết quả lớp phụ trách |
...
- Tạo bài kiểm tra với tên, mô tả, form provider, form URL hoặc mode `internal`.
- Moderator tạo và quản lý ngân hàng đề thi theo từng học phần; giảng viên chỉ được chọn lại câu hỏi khả dụng để gắn vào bài kiểm tra nội bộ của lớp học.
- Câu hỏi ngân hàng đề có metadata `CLO`, `Chương`, `Mức độ`, `Điểm`, trạng thái `Khả dụng` và ghi chú nội bộ cho người soạn đề.
- Với câu hỏi trắc nghiệm, Mod có thể thêm nhiều đáp án thay vì bị giới hạn cố định 4 lựa chọn.
- Khi giảng viên tạo bài kiểm tra `internal`, chỉ hiển thị các câu hỏi ngân hàng đề thuộc đúng học phần, đang `Khả dụng` và khớp `CLO` của thành phần đánh giá đã chọn.
- Khi giảng viên chọn câu hỏi cho bài kiểm tra `internal`, UI hiển thị thêm ba bảng thống kê nhỏ theo `CLO`, `Chương`, `Mức độ` với số câu, điểm và tỷ lệ theo điểm.
...
- Nếu `delivery_mode = external`, khối chọn câu hỏi từ ngân hàng đề bị ẩn hoàn toàn.
- Nếu `delivery_mode = internal`, bài kiểm tra dùng snapshot câu hỏi của học phần; giảng viên không nhập link form ngoài.
...
- Chỉ Mod được tạo, cập nhật, đổi trạng thái `Khả dụng` hoặc xóa câu hỏi khỏi ngân hàng đề học phần.
- `Ghi chú nội bộ` của câu hỏi chỉ phục vụ Mod khi soạn đề, không hiển thị cho sinh viên trong lúc làm bài hay xem kết quả.
- `Mức độ` của câu hỏi dùng 6 mức Bloom cải tiến: `Nhớ`, `Hiểu`, `Vận dụng`, `Phân tích`, `Đánh giá`, `Sáng tạo`.
- Điểm mặc định của câu hỏi được map theo mức độ: `Nhớ = 1`, `Hiểu = 2`, `Vận dụng = 4`, `Phân tích = 6`, `Đánh giá = 8`, `Sáng tạo = 10`, nhưng Mod vẫn có thể chỉnh tay.
- Ràng buộc dạng câu hỏi và mức độ:
  - `Đúng/Sai`: chỉ `Nhớ` hoặc `Hiểu`.
  - `Nhiều lựa chọn`: `Nhớ`, `Hiểu`, `Vận dụng`.
  - `Nhiều đáp án`: `Nhớ`, `Hiểu`, `Vận dụng`, `Phân tích`.
  - `Trả lời ngắn`: `Vận dụng`, `Phân tích`, `Đánh giá`.
  - `Tự luận`: `Phân tích`, `Đánh giá`, `Sáng tạo`.
...
- Website đã hoàn thiện phần lõi quản lý học phần, lớp học, tài liệu, mô phỏng, thư viện, dashboard và bài kiểm tra external/internal theo snapshot câu hỏi.
