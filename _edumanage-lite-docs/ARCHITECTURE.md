...
```text
app/
  (auth)/            Đăng nhập và đăng ký sinh viên
...
Sinh viên được phép tự đăng ký tài khoản, nhưng quyền truy cập học tập phải đi qua trạng thái duyệt ở tầng nghiệp vụ (`pending_approval` -> `active`).

Tài khoản `teacher` và `moderator` không đi qua self-signup; được tạo bởi `admin` trong module User management.
...
| `moderator` | Quản lý học phần trong phạm vi được admin cấp, duyệt yêu cầu theo scope và gửi thông báo chung |
| `admin` | Toàn quyền hệ thống, quản lý tài khoản, quota thư viện cá nhân, cấp scope và duyệt yêu cầu cấp cao |
...
Phân quyền moderator không phải quyền toàn cục theo role đơn lẻ. Hệ thống phải có lớp `permission_scope` để ràng buộc quyền theo `course` hoặc `class`.
...
4. Moderator có quyền tương đương giảng viên trong phạm vi scope được admin cấp.
5. Admin có thể tạo, vô hiệu hóa, thu hồi quyền teacher/moderator.
6. Chỉ giảng viên phụ trách lớp được duyệt yêu cầu tham gia lớp.
7. Admin không tạo lớp trực tiếp trong workflow chuẩn; giảng viên gửi yêu cầu mở lớp, Mod/Admin duyệt.
8. Mỗi học phần có ngân hàng đề riêng và kho kết quả kiểm tra riêng.
9. Kết quả assessment của lớp phải được mirror về kho kết quả cấp học phần.
...
| E2E test | Luồng giảng viên gửi yêu cầu mở lớp, tạo tài liệu và sinh viên truy cập |
